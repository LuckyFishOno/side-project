...
app.post('/api/login', async (req, res) => {
    ...
    console.log('收到新的註冊請求：', { name, email, password });

    if(name && email && password) 
        res.status(200).json({ 'message': '註冊成功' }); 
    else
        res.status(400).json({ error: '註冊失敗，請檢查輸入' });
})
...
app.post('/api/ticket/1', async (req, res) => {
    ...
    console.log('livenationMode:', livenationMode);
    console.log('logingAccountLN:', logingAccountLN);
    console.log('logingPasswordLN:', logingPasswordLN);
    console.log('eventUrlLN:', eventUrlLN);

    console.log('eventUrl (info):', eventUrl);
    console.log('buyTime (info):', buyTime);
    console.log('eventNumber (info):', eventNumber);
    console.log('sessionPrice (info):', sessionPrice);
    console.log('sessionSeat (info):', sessionSeat);
    console.log('ticketCount (info):', ticketCount);

    console.log('serialMode (advanced):', serialMode);
    console.log('serialCard (advanced):', serialCard);
    console.log('mooncakeMode (advanced):', mooncakeMode);
    console.log('seatWebMode (advanced):', seatWebMode);
    console.log('seatWebUrl (advanced):', seatWebUrl);
    console.log('buyTicketQuickPrice (advanced):', buyTicketQuickPrice);
    console.log('buyTicketQuickSeat (advanced):', buyTicketQuickSeat);
    console.log('buyTicketQuickTicket (advanced):', buyTicketQuickTicket);
    console.log('-------------------------------------------');
    ...
    if(cookie) {
        ...
        res.status(400).json({ error: '輸入失敗' });
    }
})

app.post('/api/ticket/3', async (req, res) => {
    ...
    if(golbalCancle) 
        ...
        res.status(400).json({ error: '取消失敗' });
})

// 用於儲存 SSE 連線的 response 物件
app.get('/api/ticket/2', (req, res) => {
...
async function main(cookie, livenationMode, logingAccountLN, logingPasswordLN, eventUrlLN, eventUrl, buyTime, eventNumber, sessionPrice, sessionSeat, ticketCount, serialMode, serialCard, mooncakeMode, seatWebMode, seatWebUrl, buyTicketQuickPrice, buyTicketQuickSeat, buyTicketQuickTicket) {
    ...
    sseResponse.write(`data: ${'##### Start Main Bot #####'}\n\n`);
    ...
    try {
        ...
        if(golbalCancle == 'botStop') {
            ...
            sseResponse.write(`data: ${'Bot Canceled !'}\n\n`);
            golbalCancle = '';
            await browser.close();
        }

        sseResponse.write(`data: ${'Tixcraft Login Success !'}\n\n`);
        ...
        if(livenationMode == true) {
            ...
            await setDelayTime(5000);
            sseResponse.write(`data: ${'Live Nation Login Success !'}\n\n`);
        ...
        gotoEvent: while(door) {
            ...
            sseResponse.write(`data: ${'Start Buy Ticket'}\n\n`);
            ...
            if(gotoSeatChk == false) {
                if(seatWebMode == true) {
                    ...
                    if(homeChk != '') {
                        ...
                        sseResponse.write(`data: ${'----- Reloading Seat Page -----'}\n\n`);
                        continue gotoEvent;
                    }
            ...
            gotoSeat: while(true) {
                ...
                if(mooncakeMode == false) {
                    ...
                    if(Ticket == '') {
                        ...
                        sseResponse.write(`data: ${'------reloading------ ' + reloadCount++}\n\n`);
                        continue gotoSeat;
                    }
                ...
                gotoSubmit: while(true) {
                    ...
                    if(verifyCodeManual == false) {
                        ...
                        sseResponse.write(`data: ${'VerifyCode : ' + temp}\n\n`);
        ...
        if(golbalCancle=='botStop' && door==true) {
            ...
            console.log('Success');
            buySuccessRes = true;
            sseResponse.write(`data: ${'Success'}\n\n`);
            sseResponse.write(`data: ${'Success'}\n\n`);
            sseResponse.write(`data: ${'Success'}\n\n`);
